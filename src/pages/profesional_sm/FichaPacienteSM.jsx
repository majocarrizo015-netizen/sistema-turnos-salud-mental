...
export default function FichaPacienteSM() {
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        <div className="bg-page-bg rounded-2xl border border-border p-4">
          ...
          <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm">
            <div><p className="text-xs text-text-secondary">Apellido y Nombre</p><p className="text-text-primary font-medium">{paciente?.apellido}, {paciente?.nombre}</p></div>
            <div><p className="text-xs text-text-secondary">DNI</p><p className="text-text-primary">{paciente?.dni}</p></div>
            <div><p className="text-xs text-text-secondary">Fecha de Nacimiento</p><p className="text-text-primary">{paciente?.fecha_nacimiento ? new Date(paciente.fecha_nacimiento + 'T00:00:00').toLocaleDateString('es-AR') : '—'}</p></div>
        ...
        <div className="bg-white rounded-2xl border border-border p-4">
          ...
          <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm mb-3">
            ...
            <div><p className="text-xs text-text-secondary">Matrícula</p><p className="text-text-primary">{profesional?.matricula || '—'}</p></div>
          </div>
          <div className="flex flex-wrap gap-2 mb-3">
            <span className="px-2.5 py-0.5 rounded-full text-xs bg-profesional-sm-light text-profesional-sm-primary border border-profesional-sm-primary font-medium">
